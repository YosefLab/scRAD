...
attempt to relate cell expression profiles to each other based on their pairwise
...
ratios. Distances computed on these features may be used partition or cluster
...
`k` that maximizes the "overall average silhouette width": the average over all 
...
expression of that gene across all cells in the study, excluding cells in `c2`.
...
surprising given that cell clustering is based on the expression data itself.
...
appears to be stricter than our significance criterion, selecting a smaller
...
synthesizing multiple analyses: we consider the set of hub genes that are 
...
differential expression condition requires specificity. Lists of hub genes and
...
Consider a knockdown microaaray data set collected by Chevrier et al. 2011. In 
...
phosphoproteins on the mRNA expression of mouse dendritic cells (DCs).
Normalized expression data for 128 genes can be downloaded from the NCBI
...
analysis presented in Gayo et al. 2017 relied on "weighted"" signatures based 
on models of missing data, here we use a simple signature equal to the opposite
of the correlation between centered single-cell and micro array profiles. We'll 
refer to these signatures as an "(Unweighted) Upstream Regulatory Scores"
because they score the distance of a cell profile from expression profiles in
which specific regulators have been inhibited.
...
analyzed bone marrow-derived dendritic cells perturbed by various TLR agonists,
...
will also only consider late perturbations and 0-hour control in the analysis 
that follows. Late perturbations may be most relevant because our single cells
were stimulated over the course of 2 days.
...
tlr_array  = tlr_array [, grepl("[^C]_24|C_0", colnames(tlr_array))]
...
Now, although we did not consider the point in Gayo et al. 2017, if we note the
similarity between this data matrix and the matrix of upstream regulatory
scores, we might consider running the same type of IDR-based differential
signature analysis as above. Unfortunately, the smaller number of signatures in
this case can complicate the underlying statistical inference. Running the same
analysis as before, we come across an error message:
...
properly; on the other hand, it may be more straight-forward to consider an 
...
While this method does not address reproducibility, it is not fit based, and is
therefore significantly more robust. Below we present an example of how to run
this command and visualize the results in a volcano plot.
...
plot(
  ...
  pch = 21,
  bg = c("black",brewer.pal(nrow(x)-1,"Set1")),
  main = paste("TLR Stimulation Analysis: c1 vs c3-5"),
  ylab = "-log10(FDR)",
  xlim = c(-0.1, 0.1),
  ylim = c(0, 20),
...
# Label
text(diff_scores[rownames(meta_kruskal_pool_tlr$kruskal_pvals)][is_sig],
...
One of the results of this analysis are that, compared to `c3-5` cells, `c1` 
cells are significantly more similar to PolyI:C induced mouse cells. This may 
suggest that PolyI:C induction could enrich for `c1` cells over `c3-5` cells, 
but we may wonder, in more absolute terms, which type of induction is most 
`c1`-like at the expense of all other cell states: `c3-5`-like and otherwise.
One way to address this problem is to analyze the distributions of TLR
induction scores for `c1`, considering replicate pools to avoid batch effects.

```{r meta_tlr2}

is_included = clustering %in% "c1"
x0 = stim_scores[, is_included]["C_0h",]
x = stim_scores[, is_included][-which(rownames(stim_scores) == "C_0h"),]
r = factor(c(rep("P12", 2), "P3")[donor[is_included]])
...
The CDFs above suggest that PolyI:C induced mouse cells might be 
most similar to `c1`. We can take advantage of a control signature (similarity 
to the control sample) in order to define 5 separate Kolmogorov–Smirnov (KS) 
tests: are any of the TLR induction signature distributions significantly
different from the control signature distribution?

The `ksMeta` function in `scider` can run these 5 tests in all 2 replicate
pools, applying the same meta-analysis procedure as `kruskalMeta`:

```{r meta_tlr2_test}

meta_ks_pool_tlr = ksMeta(x,x0,r,exact = TRUE, alternative = "less")
...
diff_scores = colMeans(mean_sigs[,-1] - mean_sigs0[,-1])
...
It would appear from the volcano plot above that PolyI:C exhibits the most 
significant positive deviation from control similarity, suggesting again that
this form of induction is a good candidate for generating `c1`-like cell
states.

## IDR-Based Reproducible Module Analysis

In the final part of this section we will introduce one more IDR-based in 
`scider`. In the previous section we have discussed multiple steps for 
...
of this graph is not particularly interesting.
...
antagonistic signature modules, including the signature of TBK1.
...
`scider` offers another low-level function, `est.IDRm.sample`, which runs IDR analysis
over various sub samples of tests. By running the analysis many times, we can evaluate the stability of our outputs. Below we can take the opportunity to compare the range of fit parameters values after we vary our initial estimates.
...
The red X's mark the fit parameter values in the previous run. The box plot 
object exported by `est.IDRm.sample` shows us that even with our new initial 
parameters, the fit robustly converges on the same final values.
